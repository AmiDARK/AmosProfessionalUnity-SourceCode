...
demult         equ 2                   ; 2019.11.21 To increas buffers.
...
CliIn:
    bsr       Reserve_DZ
    clr.b     Flag_AMOS(a5)            ; Exécution depuis le CLI
...
; Explore sa ligne de commande (si option -E)
    ...
    bne.s     .COk                     ; SI oui, go to .COk
    move.l    Path_Config(a5),a0       ; Si chemin d'accès défini en ligne de commande
    tst.b     (a0)                     ; Oui ? Non ?
    bne.s     .CLoad                   ; Le charger depuis la définition. go to .CLoad    
    lea       Def_Config0(pc),a0       ; Nom de la configuration
    move.l    Path_Config(a5),a1       ; Chemin d'accès
    bsr       CopName                  ; Copie nom (a0)->(a1)
    bsr       Load_Config              ; Essaye de charger la configuration
    bne.s     .COk                     ; Configuration Chagée, go to .COk
    lea       Def_Config1(pc),a0       ; 2nd nom de config pour chargement si 1er fail
    move.l    Path_Config(a5),a1       ; Chemin d'accès
    bsr       CopName                  ; Copie nom (a0)->(a1)
    bsr       Load_Config              ; Essaye de charger la configuration
    bne.s     .COk
    lea       Def_Config2(pc),a0       ; 3ème nom de config pour chargement si 1er et 2ème en fail.
    move.l    Path_Config(a5),a1       ; Chemin d'accès
    bsr       CopName                  ; Copie nom (a0)->(a1)
.CLoad:
    bsr       Load_Config              ; Essaye de charger la configuration
    beq       Err_CantLoadConfig       ; Erreur : Impossible de charger la configuration du compilateur
.COk:
    bsr       Cold_Config              ; Recupere les chaines par defaut
    moveq     #1,d0
...
; Charge la configuration interpreteur
    bsr       Init_Config              ; Charge la configuration de l'interpréteur Basic (extensions, buffers, etc.)
; Des erreurs?
    move.l    (sp)+,d0
    bne       Err_InCommand
    move.l    (sp)+,d0
...
Db:
    ...
    add.l     End_Source(a5),d2         ; D2 = Fin du code source
    move.l    B_Source(a5),a0           ; A0 = Pointeur du début du code source
    lea       20(a0),a0                 ; A0 = Début du code source
    moveq     #0,d3                     ; D3 = Ligne 1
DbRloop:
    moveq    #0,d1
DbLoop:
    addq.w    #1,d3
...
.SprIco:
    ...
    move.l    B_Work(a5),a1
.SLoop:
    bsr    GetWord            Read Sprite SX
    bsr    OutWord            Write Sprite SX
    move.w    d0,d3
    bsr    GetWord            Read Sprite SY
    bsr    OutWord            Write Sprite SY
    mulu    d0,d3
    bsr    GetWord            Read Sprite Depth
    bsr    OutWord            Write Sprite Depth
    mulu    d0,d3
    bsr    GetLong            Read Sprite HX/HY
    bsr    OutLong            Write Sprite HX/HY
    lsl.l    #1,d3            En bytes
    bsr    Copy_Source        Copie le sprite meme si ZERO!
    dbra    d2,.SLoop
.BVide:
    moveq    #32*2,d3        Copie la palette * 2023.01.23 A mettre a jour avec palette 256 couleurs.
    bsr    Copy_Source
; Banque suivante
; ~~~~~~~~~~~~~~~
...
GetPaletteSize:                     ; 2023 Set palette size from source
        tst.b     Flag_Source(a5)
        bne.w     DiskDot
        add.l     B_Source(a5),a6
...
CL_NoWb    
    clr.b    Flag_WB(a5)
    rts
CL_ExtraBuffers:                   // 2023.03.01 Added for larger source codes files projects (demult update)
    move.b    #1,largeBuffer(a5)
    rts
CL_Quiet
