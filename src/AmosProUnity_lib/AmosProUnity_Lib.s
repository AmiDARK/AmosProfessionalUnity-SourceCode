...
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        Include    "tmpCompil/AmosProUnity_Lib_Size.s"
...
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    dc.b "\\\\\\\\\\\\\\\","/"+$80,"I",-1 ; " <- to avoid SublimeText thinking a double comma is not closed
...
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    dc.w    .Fin-.Debut
.Debut:
    IncBin    "tmpCompil/Toktab_Verif.Bin"
.Fin
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Include    "src/AmosProUnity_lib/AmosProUnity_ILib.s"
; - - - - - - - - - - - - -
    Lib_Def    MainLibrary
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
;                     ERRN
...
; Overwrite ou append?
    tst.w    d5
    beq.s    .Over
    Rbsr    L_Bnk.GetBobs              ; Return BNE trouve A0/A1=adresse
    beq.s    .Over
; Append
    moveq    #1,d0                     ; D0 = Flag Append mode 
    move.w    d6,d1                    ; D1 = Bobs Amount from file
    move.w    (a0),d5                  ; D5 = Bobs amount in memory bank
    add.w    d5,d1                     ; D1 = Total amount of bobs (file + memory bank)
    bra.s    .Res                      ; -> Reserve bank (with append mode)
; Overwrite
.Over:
    moveq    #0,d0                     ; D0 = Flag erase/overwrite
    move.w    d6,d1                    ; D1 = Bobs amount from file
    clr.w    d5                        ; D5.w = 0
; Va reserver la place
.Res:
    Rbsr    L_Bnk.ResBob               ; a1 = Banque origine (si definie), d0 = Flag, d1 = Nombre, d2 = Flags
    bne    LB_MErr
...
; - - - - - - - - - - - - -
    ...
    tst.w    FillF32(a5)
    bne.s    .Skip
    lea    8(a2),a0        Si pas de 32, longueur réelle!
.Long    tst.b    (a0)+
    bne.s    .Long
    sub.l    a2,a0
    move.l    a0,d0
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Lib_Def    Bnk.GetBobs
; - - - - - - - - - - - - -
    movem.l    d0/d1,-(sp)             ; SaveRegs
    moveq    #1,d0                     ; D0 = 1 = Type of bank to retrieve ( 1 = Bobs/Sprites )
    Rbsr    L_Bnk.GetAdr               ; A1 = Bank Adress of found Bank Type 1, A0 = A1 + 24
    beq.s    .Nof                      ; A1 = 0 = Not Exist -> Jump .nof
    btst    #Bnk_BitBob,d0             ; Test Bit #Bnk_BitBob in DO
.Nof:
    movem.l    (sp)+,d0/d1             ; LoadRegs
    rts
...
; - - - - - - - - - - - - -
    movem.l    d0/d1,-(sp)             ; SaveRegs
    moveq      #2,d0                   ; D0 = 2 = Type of bank to retrieve ( 1 = Icons )
    Rbsr       L_Bnk.GetAdr            ; A1 = Bank Adress of found Bank Type 1, A0 = A1 + 24
    beq.s      .Nof                    ; A1 = 0 = Not Exist -> Jump .nof
    btst       #Bnk_BitIcon,d0         ; Test Bit #Bnk_BitBob in DO
.Nof:
    movem.l    (sp)+,d0/d1             ; LoadRegs
    rts
...
; - - - - - - - - - - - - -
    Rbsr       L_Bnk.GetIcons      ; A0 = A1 = Icon Palette Bank
    Rbeq       L_BkNoRes
    move.w     (a0)+,d0
...
;; - - - - - - - - - - - - -
    Lib_Def    GSPal
    AmpLCallR  A_GSPal,a1 ; 2021.02.16 ReTested OK
    rts
;; - - - - - - - - - - - - -
    Lib_Def    PalRout
;; - - - - - - - - - - - - -
    AmpLCallR  A_PalRout,a1 ; 2021.02.16 Tested OK.
    rts
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
;    Initialisation à chaud des dialogues
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
.In1    move.l    Arx_Port(a5),a0
    ...
    tst.l    d0
    bne.s    .Loop1
; Détruit le port de communication
...
Fs_Help    bsr    Fs_GetFName
    ...
    moveq    #0,d0
    tst.b    FillFSorted(a5)        Si pas sorted, à partir du courant!
    bne.s    .Sl
; SORT pas en route
...
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    tst.b    (a1)
    beq    Fs_Cancel
; Copie le nom à la suite de name 1
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    move.l    Name1(a5),a0
.Fin    tst.b    (a0)+
    bne.s    .Fin
...
; ___________________________________________________________________
;
;     Fermeture du sélecteur de fichier / Sortie
; ___________________________________________________________________
;
...
; Recupere la position de l''écran
...
; On revient!
; ~~~~~~~~~~~
    movem.l    (sp)+,a0/d0        Recupere le résultat
...
Fs_Store
    ...
    bne.s    .Exit
    tst.l    BufFillF(a5)        Quelque chose à stocker?
    beq.s    .Exit
...
; Effacement de tous les noms de l''écran
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.NmD6    move.b    (a0)+,(a1)+
    ...
    move.l    d6,a0
    clr.b    (a0)
; Vérifie la présence du / au début...
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
; Insere le directory A0 à la suite du directory courant (NAME1-NAME2)
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Fs_NewDir
    move.l    a0,-(sp)
    bsr    Fs_New
...
.Pat1    move.l    (a3)+,FsV_File(a1)    Nom par défaut
...
;
;
; Initialisation de l''éditeur ligne
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;    A0=     Buffer de donnees
;    A1=    Chaine à imprimer
...
; ~~~~~~~~~~~~~~~
    ...
    cmp.b    #32,d1            
    bcc.s    .Lt
; Retour à l''appelant, avec le scancode eventuel en D1
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
.Out    btst    #LEd_FKeys,LEd_Flags(a3)
    beq    .Loop
    moveq    #0,d2
    bra.s    .Exit
; Rien en retour. Retourner à l''appelant?
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.G    move.w    LEd_Cur(a3),d2
    ...
    bne.s    .Gl
    subq.w    #1,LEd_Cur(a3)        ; Un caractere à gauche
    bra    .Print
...
; Impression de la ligne, retour à la boucle
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~
    Rjsr    L_Dia_ClearVar
; Stocke le Numero / Adresse de l''écran courant
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    move.l    T_EcCourant(a5),a0
...
.IOk    move.w    d1,(a3)+        Adresse dans la liste
    ...
    addq.l    #4,a3
    Rbsr    L_Dia_FChif
    move.l    (a3),d1            Jusqu''à 16 parametres
    cmp.l    #9,d1
    bhi    .Synt
    move.w    d1,(a3)+
...
; ___________________________________________________________________________
;
;    Active l''écran d''une boite de dialogue
; ___________________________________________________________________________
;
; IN
...
; - - - - - - - - - - - - -
    movem.l    a0-a1/d1,-(sp)
; Verifie la presence de l''écran, du meme!
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
; - - - - - - - - - - - - -
    movem.l    d0-d1/a0-a1,-(sp)
; Une fenetre d''édition en route?
...
; - - - - - - - - - - - - -
    ...
    move.l    d0,d5
    move.l    d1,d4
; Va effacer une boite déja en route
    Rbsr    L_Dia_EffChannel
...
; Initialise pour les tests
    ...
    move.w    Dia_Error(a4),d0    Pas d''erreur, revient directement
    beq.s    .OOut
.Err    move.l    a4,a0            Erreur, efface ce qui a ete dessiné
...
; Un RUn a eu lieu!
; ~~~~~~~~~~~~~~~~~
.Runné    move.l    a4,a0            Efface le canal
    Rbsr    L_Dia_EffChanA0
...
; Active l''ancien ecran
    Rbsr    L_Dia_Active
    bne    .PaEff
; Efface les fenetres / Zones réservées par le programme
    clr.l    -(sp)
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Lib_Def    Dia_FTokens
; - - - - - - - - - - - - -
Dia_FTokens    IncBin    "src/bin/Dialog_Funcs.bin"
        Even
...
; ________________________________________________________________________
;
;    Interface avec l''éditeur ligne
;
...
; Reserve le buffer
    ...
    pea    4(a0)
; Copie la chaine de départ, finie par zero
    Rbsr    L_Dia_Evalue
    move.l    d0,a0
; Flags
    move.l    (sp)+,a1
...
; Cree la fenetre
    ...
    add.w    #1000,d1        Fenetres à partir de 1000
    move.w    d1,Dia_WindOn(a4)
    WiCall    WindOp
; Paper / Pen
    Rbsr    L_Dia_Evalue
...
; Reserve la zone / Initialise
    ...
    WiCall    Print
; Imprime le texte, curseur à la fin
    movem.l    (sp)+,a1/d0/d2
    moveq    #0,d1
    moveq    #-1,d3
    lea    Dia_LEd(a2),a0
...
; Cree la fenetre
    ...
    sub.l    a1,a1
    move.w    Dia_ZoNumber(a2),d1
    add.w    #3000,d1        Fenetres à partir de 3000
    move.w    d1,Dia_WindOn(a4)
    WiCall    WindOp
    tst.w    d0
    Rbne    L_Dia_Fonc
...
; Cree la fenetre
    ...
    sub.l    a1,a1
    move.w    Dia_ZoNumber(a2),d1
    add.w    #2000,d1        Fenetres à partir de 1000
    move.w    d1,Dia_WindOn(a4)
    WiCall    WindOp
    tst.w    d0
    Rbne    L_Dia_Fonc
...
; Peut-on effectuer l''instruction?
    move.l    Dia_Vars-3*4(a4),d0        Si appelé en dehors de la parenth
    Rbeq    L_Dia_Synt
    move.l    d0,a0
...
; Peut-on effectuer l''instruction?
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    tst.l    Dia_Vars-3*4(a4)        Si appelé en dehors d''une parenth
    Rbeq    L_Dia_Synt
    move.l    #EntNul,d4
    move.l    d4,d5
    Rbsr    L_Dia_ZUpdate
...
Dia_FVar    
    ...
    move.l    0(a0,d0.l),(a3)
    rts
; = ZV Une variable interne à une zone
; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Dia_FZVar
    addq.w    #1,d7
    move.l    Dia_Vars-3*4(a4),d0
    beq.s    .Ext
; Variable à l''interieur de la zone
    move.l    d0,a0
    move.l    Dia_ZoVar(a0),-(a3)
    rts
; Variable à l''exterieur de la zone
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
;     FONCTION: Conversion en décimal CHIF #
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
.Skip    rts
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
;     Passe paper/pen à zero pour effacement
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Lib_Def    Dia_WiZero
; - - - - - - - - - - - - -
    lea    .Zero(pc),a1
...
; Saute la partie reponse
    ...
    move.l    a1,d0
    sub.l    d6,d0
    move.b    d0,6(a3)            Debut mot clé dans texte
    bset    #31,d4
    cmp.b    #",",(a1)            Si pas de mot clé
    bne.s    .ALoop
    bclr    #31,d4
.ALoop    move.b    (a1)+,d0
    beq    .AErr
...
; Ok!
    ...
    rts
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
;     Affichage d''un des éléments de la liste - A2 structure - D0=#
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Lib_Def    Dia_LiAff
; - - - - - - - - - - - - -
...
; - - - - - - - - - - - - -
    moveq    #127,d0                Teste jusqu''à 127 boites
...
.Loop    move.l    d0,a4
    lea    8(a4),a4
    btst    #0,Dia_RFlags(a4)        Canal effacé?
    beq.s    .Next
    btst    #2,Dia_RFlags(a4)        Canal FREEZE?
    bne.s    .Next
    Rbsr    L_Dia_Active            Active l''écran
    bne.s    .Err
...
.PaRet    tst.l    d1            Une touche?
    ...
    cmp.b    #$42,d0            Tab?
    bne.s    .EdK
; Trouve la prochaine fenetre à éditer
.EdNxt    move.l    a2,a0
...
.Pa4    move.l    Dia_KyZone(a2),d0
    ...
    move.l    d0,d5            Pointe l''objet à activer
    moveq    #0,d6
    moveq    #0,d7
    bra    .MousIn
; Passe au suivant
...
; - - - - - - - - - - - - -
    SyCall    XyMou            Coordonnées du curseur
    moveq    #0,d3
    SyCall    XyScr            Dans l''écran
    Rbra    L_Dia_GetZ
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
;     Activation de la premiere fenetre d''édition
; - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Lib_Def    Dia_EdFirst
; - - - - - - - - - - - - -
    move.l    Dia_Buffer(a4),a0
...
.Codes    dc.b    %00100        *  0 " " Fin
    ...
    dc.b    %00001        * 90 "Z"
    dc.b    %00011        * 91 "["
    dc.b    %00011        * 92 "\" ; " <- Because Sublime text take \ as a comma cancelor (Java, etc.)
    dc.b    %00011        * 93 "]"
    dc.b    %00011        * 94 "^"
    dc.b    %00011        * 95 " "
    dc.b    %00000        * 96 "`"
...
;_____________________________________________________________________________
;
;                    Gestion généralisee des boutons
;_____________________________________________________________________________
;
...
;_____________________________________________________________________________
;
;                    Gestion généralisee des sliders.
;_____________________________________________________________________________
;
; __________________________________________________
;
