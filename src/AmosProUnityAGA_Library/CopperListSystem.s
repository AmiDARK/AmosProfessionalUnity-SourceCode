...
; *************************************************************
EcForceCop:
    ScreenReplacementM ForceFullCopperRefresh,a0 ; 2020.10.14 replace EcForceCop call
    addq.w     #1,T_EcYAct(a5)         ; Enforce re-calculation for screens.
EcCopper:                
    ScreenReplacementM CopperRefresh,a0 ; 2020.10.14 replace EcCopper call
...
* Rainbow?
    tst.w      T_RainBow(a5)
    bne.s      CopBow                  ; If at least 1 RAINBOW is created/active, then jump to Rainbow update
...
MCopX:
    ...
    bsr        EcCopBa    
.Skip
    move.l     #$FFFFFFFE,(a1)+        ; Insert the last line of the Copper List.IWS
...
; *************************************************************
EcCopHo:
    ScreenReplacementM InsertScreenInCopper,a4  ; 2020.10.12 Use Plugin InsertScreenInCopper instead of EcCopHo when available.   
...
; *************************************************************
EcCopBa:
  ScreenReplacementM EndOfScreenCopper,a4 ; 2020.10.14 Replace EcCopBa call
;    move.w     #BplCon3,(a1)+         ; 2019.11.05 Update BplCon3 when closing screen
;    move.w     #0,(a1)+               ; Reset everything concerning BplCon3.
    move.w     d0,d2                   ; d2 = The last Y line of the screen in the copper list
    sub.w      #EcYBase,d2             ; d2 = d2 - EcYBase
    bsr        WaitD2                  ; Call WaitD2 to makes copper list wait until line D2
    move.w     #DmaCon,(a1)+           ; DMACON register to modify
    move.w     #$0100,(a1)+            ; Clear DMACon register
    move.w     #Color00,(a1)+          ; COLOR00 register to modify
    move.w     T_EcFond(a5),(a1)+      ; COLOR00 = Background color defined for the screens
    rts
...
WaitD2:
    cmp.w      #256,d2                 ; d2 = 256 ?
    bcs.s      WCop                    ; Line 256 Not yet reached -> Jump WCop
    tst.w      T_Cop255(a5)            ; Is T_Cop255(a5) = 0 ?
    bne.s      WCop                    ; No -> Jump WCop
    move.w     #$FFDF,(a1)+            ; set line Y.b=255, X.b=#223
    move.w     #$FFFE,(a1)+            ; Wait
    addq.w     #1,T_Cop255(a5)         ; T_Cop255(a5) + 1
WCop:
    lsl.w      #8,d2                   ; D2 = D2 * 256
    or.w       #$03,d2                 ; D2 = D2 + $03
    move.w     d2,(a1)+                ; set line Y.b=d2, X.b=$03
    move.w     #$FFFE,(a1)+            ; Wait
    rts
...
CpInit:    
    ScreenReplacement  RestartWithAGACopperList ; 2020.10.12 Use Plugin RestartWithAGACopperList instead of CpInit when available.   
...
******* COPPER ON/OFF
TCopOn:
    ScreenReplacementM CopperOnOff,a0  ; 2020.10.14 Replacement for TCopOn
    tst.w      d1
    bne.s      ICpo1
* Copper OFF -> Hide!
    tst.w      T_CopON(a5)
...
******* COPSWAP
TCopSw:
    ScreenReplacementM CopperSwap,a0
    tst.w      T_CopON(a5)
    bne        CopEr1
    move.l     T_CopPos(a5),a0
    move.l     #$FFFFFFFE,(a0)
TCpSw:
    ScreenReplacementM CopperSwapInternal,a0
    move.l     T_CopLogic(a5),a0
    move.l     T_CopPhysic(a5),a1
    move.l     a1,T_CopLogic(a5)
    move.l     a0,T_CopPhysic(a5)
